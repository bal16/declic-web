...
- Additional upload/engagement guards:
  - if selected `exhibitions.phase === 'ARCHIVED'`, `/dashboard/upload` shows `"Exhibition has been archived, new uploads are closed"` (API also returns `403 {code:"ARCHIVED"}`).
  - if `feature_flags.series_enabled === false`, upload form hides SERIES toggle/batch UI and shows `"SERIES creation is temporarily disabled"` on attempt (API returns `403 {code:"FEATURE_DISABLED"}`). Existing SERIES works remain visible.
