...
**Org:** bal16
**Deciders:** repo owner
**Related:** [PRD](../specs/PRD.md) §6/§8.5, [PRD-API](../specs/PRD-API.md) §1–§2/§4, [PRD-Worker](../specs/PRD-Worker.md) §1.2, [db-schema](../data/db-schema.md), `../data/seed.ts`, `../ops/docker-compose.yml`, [DEVELOPMENT](../guides/DEVELOPMENT.md)
...
* `apps/web` — TanStack Start public gallery + photographer dashboard + admin UI ([PRD-FE](../specs/PRD-FE.md) §2).
* `apps/api` — NestJS on Bun, owns Better Auth sessions, RBAC, presigned MinIO uploads, and enqueues one BullMQ job per `photo_items` row ([PRD-API](../specs/PRD-API.md) §1/§4).
...
* Deploy constraint is orthogonal but recorded here: Better Auth cookies require web + API under one registrable domain (`app.*` + `api.*` with `trustedOrigins`/CORS) or an `/api/*` reverse proxy through the web domain ([PRD](../specs/PRD.md) §8.5). Splitting repos does not split domains.
...
* **Web deploy target is TBD — Vercel explicitly out**
  ([DEVELOPMENT](../guides/DEVELOPMENT.md) §9.2). The `Vercel` cell in the §2
...
* Repo layout, dev commands, and deploy flow: [DEVELOPMENT](../guides/DEVELOPMENT.md)
* Product vision and roles: [PRD](../specs/PRD.md)
* API schema and endpoints: [PRD-API](../specs/PRD-API.md)
* Worker pipeline: [PRD-Worker](../specs/PRD-Worker.md)
* Canonical schema diagram: [db-schema](../data/db-schema.md)
* Seeds: `../data/seed.ts` (moving to `packages/db/src/seed.ts`; see [DEVELOPMENT](../guides/DEVELOPMENT.md))
