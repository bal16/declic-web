...
## 1. Tokens (`@theme inline`, dark in `:root`)

Follow shadcn convention for Tailwind v4: semantic CSS variables mapped
through `@theme inline` in `apps/web/src/styles.css`. Dark-first means
the dark exhibition theme lives directly in `:root` — there is no
`.dark` class and no light theme in v1 (adding one later = new `:root`
values under `.light`, out of scope).

Palette (from PRD-FE §4.1, values unchanged):
...
| `--primary` | `43 74% 49%` (`#D4AF37` gold) | curatorial accents ONLY (CTA, active, focus ring) — never large backgrounds |
...
| `--ring` | `43 74% 49%` | focus ring (see §4) |
| `--radius` | `0.5rem` | |

Rules: `--primary` never backgrounds; works/covers always sit on
`--background`; `SERIES` badge = `--secondary` bg + `--secondary-foreground`
text; `CURATED` badge = gold.
...
1. **Contrast ≥ 4.5:1 for text.** Gold `#D4AF37` on near-black MUST pass
